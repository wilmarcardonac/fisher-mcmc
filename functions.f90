    ...
    Else 
        ...
        If (fiducial_flag) then
            ...
            Else
                ...
                open(UNIT_FILE2, file='./ini_files/El_nl.ini')

                write(UNIT_FILE2,*) 'root = ../data/El_nl_'

                write(UNIT_FILE2,'(a25)') 'number count error = 0.10'   
        ...
        Else
            ...
            If (Cl_flag) then
                ...
                open(UNIT_FILE2, file='./ini_files/Cl_'//trim(string)//'.ini')

                write(UNIT_FILE2,*) 'root = ../data/Cl_'//trim(string)//'_'
        ...
        write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'

    End if
    
    ! Background parameters and anisotropic stress

    write(UNIT_FILE2,'(a6, es16.10)') 'A_s = ', A_s  

    write(UNIT_FILE2,'(a6, es16.10)') 'n_s = ', n_s

    write(UNIT_FILE2,'(a5, es16.10)') 'H0 = ', H0

    write(UNIT_FILE2,'(a10, es16.10)') 'omega_b = ', omega_b

    write(UNIT_FILE2,'(a12, es16.10)') 'omega_cdm = ', omega_cdm

    write(UNIT_FILE2,'(a11, es16.10)') 'tau_reio = ', tau

    ! Parameters for massive neutrinos                                                                                            

    write(UNIT_FILE2,'(a7, f5.3)') 'N_ur = ', real(N_ur)

    write(UNIT_FILE2,'(a9, f5.3)') 'N_ncdm = ', real(N_ncdm)

    write(UNIT_FILE2,'(a11, f5.3)') 'deg_ncdm = ', real(deg_ncdm)

    write(UNIT_FILE2,'(a9, es16.10)') 'm_ncdm = ', m_ncdm

    ! Number counts in the output                                                                                            

    write(UNIT_FILE2,'(a12)') 'output = nCl'
    
!    write(UNIT_FILE2,'(a20)') 'non linear = halofit'
 
    write(UNIT_FILE2,'(a32)') 'dNdz_selection = analytic_euclid'

    write(UNIT_FILE2,'(a32)') 'dNdz_evolution = analytic_euclid'

    write(UNIT_FILE2,'(a20)') 'selection = gaussian'

    write(UNIT_FILE2,'(a17, 4(f10.8, a1),f10.8)') 'selection_mean = ', z_bin_centers(1),',', z_bin_centers(2),',',&
    ...
    write(UNIT_FILE2,'(a18, 4(f10.8, a1),f10.8)') 'selection_width = ', z_bin_widths(1),',',z_bin_widths(2),',',&
         z_bin_widths(3),',',z_bin_widths(4),',',z_bin_widths(5)!,',',z_bin_widths(6),',',z_bin_widths(7),',',z_bin_widths(8),',',&
!    z_bin_widths(9),',',z_bin_widths(10)

    write(UNIT_FILE2,'(a17, 4(f10.8, a1),f10.8)') 'selection_bias = ', z_bin_bias(1),',',z_bin_bias(2),',',&
         z_bin_bias(3),',',z_bin_bias(4),',',z_bin_bias(5)!,',',z_bin_bias(6),',',z_bin_bias(7),',',z_bin_bias(8),',',&
!    z_bin_bias(9),',',z_bin_bias(10)

    write(UNIT_FILE2,'(a31, 4(f10.8, a1),f10.8)') 'selection_magnification_bias = ', s_z_mag_bias(1),',',s_z_mag_bias(2),',',&
         s_z_mag_bias(3),',',s_z_mag_bias(4),',',s_z_mag_bias(5)!,',',z_bin_bias(6),',',z_bin_bias(7),',',z_bin_bias(8),',',&
!    z_bin_bias(9),',',z_bin_bias(10)

    write(UNIT_FILE2,'(a15,i2)') 'non_diagonal = ',nbins-1

    write(UNIT_FILE2,'(a13)') 'headers = yes'

    write(UNIT_FILE2,'(a17)') 'bessel file = yes'

    write(UNIT_FILE2,'(a12,i4)') 'l_max_lss = ', lmax_class

    write(UNIT_FILE2,'(a8,i1)') 'l_min = ', lmin

!    write(UNIT_FILE2,'(a27,f2.0)') 'selection_magnitude_bias = ', 0.

    write(UNIT_FILE2,'(a14)') 'format = class'

    write(UNIT_FILE2,'(a17)') 'gauge = newtonian'

    ! PRECISION PARAMETERS

!    write(UNIT_FILE2,'(a40, f6.0)') 'l_switch_limber_for_cl_density_over_z = ', real(l_switch_limber_for_cl_density_over_z)

    write(UNIT_FILE2,'(a28, f5.2)') 'selection_sampling_bessel = ', real(bessel)

    write(UNIT_FILE2,'(a12, f5.1)') 'q_linstep = ', real(q)

    write(UNIT_FILE2,'(a24, f5.2)') 'k_max_tau0_over_l_max = ', real(kmaxtau0)
...
!        write(20,*) indexl,det_the, det_obs, det_mix
!        print *, indexl, 'the : ', det_the, 'obs : ', det_obs, 'mix : ', det_mix
...
!  print *, '-chi2/2 : ', euclid_galaxy_cl_likelihood
!  stop
...
!    f7 = (param_MG_beta2 == MG_beta2)
    ...
    If (Cl_El_flag  .and. fiducial_model) then
        if (len_flag) then
            open(UNIT_FILE2, file='./ini_files/fiducial_euclid_galaxy.ini')
            write(UNIT_FILE2,'(a40)') 'root = ../output/fiducial_euclid_galaxy_'
            write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
        else
            open(UNIT_FILE2, file='./ini_files/fiducial_euclid_galaxy_no_lensing.ini')
            write(UNIT_FILE2,'(a51)') 'root = ../output/fiducial_euclid_galaxy_no_lensing_'
            write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler' 
        end if
    else if (fiducial_model .and. .not.Cl_El_flag) then
        if (len_flag) then
            open(UNIT_FILE2, file='./ini_files/El_fiducial_euclid_galaxy.ini')
            write(UNIT_FILE2,'(a43)') 'root = ../output/El_fiducial_euclid_galaxy_'
            write(UNIT_FILE2,'(a25)') 'number count error = 0.10'   
            write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'     
        else
        ...
        if ((.not.f1 .and. f2) .and. (f3 .and. f4) .and. (f5 .and. f6)) then 
            if (len_flag) then 
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_omega_b_'//trim(string_omega_b)//'_lensing.ini')
                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_omega_b_'//trim(string_omega_b)//'_lensing_'
            else 
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_omega_b_'//trim(string_omega_b)//'.ini')
                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_omega_b_'//trim(string_omega_b)//'_'
            end if
        else if ((f1 .and. .not.f2) .and. (f3 .and. f4) .and. (f5 .and. f6)) then
            if (len_flag) then
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_omega_cdm_'//trim(string_omega_cdm)//'_lensing.ini')
                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_omega_cdm_'//trim(string_omega_cdm)//'_lensing_'
            else 
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_omega_cdm_'//trim(string_omega_cdm)//'.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_omega_cdm_'//trim(string_omega_cdm)//'_'
                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'
            end if
        else if ((f1 .and. f2) .and. (.not.f3 .and. f4) .and. (f5 .and. f6)) then 
            if (len_flag) then
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_n_s_'//trim(string_n_s)//'_lensing.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_n_s_'//trim(string_n_s)//'_lensing_'
                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
            else 
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_n_s_'//trim(string_n_s)//'.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_n_s_'//trim(string_n_s)//'_'
                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'
            end if
        else if ((f1 .and. f2) .and. (f3 .and. .not.f4) .and. (f5 .and. f6)) then 
            if (len_flag) then
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_A_s_'//trim(string_A_s)//'_lensing.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_A_s_'//trim(string_A_s)//'_lensing_'
                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
            else 
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_A_s_'//trim(string_A_s)//'.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_A_s_'//trim(string_A_s)//'_'
                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'
            end if
        else if ((f1 .and. f2) .and. (f3 .and. f4) .and. (.not.f5 .and. f6)) then 
            if (len_flag) then 
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_H0_'//trim(string_H0)//'_lensing.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_H0_'//trim(string_H0)//'_lensing_'
                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
            else
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_H0_'//trim(string_H0)//'.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_H0_'//trim(string_H0)//'_'
                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'
            end if
        else if ((f1 .and. f2) .and. (f3 .and. f4) .and. (f5 .and. .not.f6)) then 
            if (len_flag) then 
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_m_ncdm_'//trim(string_m_ncdm)//'_lensing.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_m_ncdm_'//trim(string_m_ncdm)//'_lensing_'
                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
            else 
                open(UNIT_FILE2, file='./ini_files/Cl_euclid_galaxy_m_ncdm_'//trim(string_m_ncdm)//'.ini')
                write(UNIT_FILE2,*) 'root = ../output/Cl_euclid_galaxy_m_ncdm_'//trim(string_m_ncdm)//'_'
                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'
            end if
        else 
    ...
    write(UNIT_FILE2,'(a6, es16.10)') 'A_s = ', param_A_s  

    write(UNIT_FILE2,'(a6, es16.10)') 'n_s = ', param_n_s
 
    write(UNIT_FILE2,'(a5, es16.10)') 'H0 = ', param_H0

    write(UNIT_FILE2,'(a10, es16.10)') 'omega_b = ', param_omega_b

    write(UNIT_FILE2,'(a12, es16.10)') 'omega_cdm = ', param_omega_cdm

    write(UNIT_FILE2,'(a11, es16.10)') 'tau_reio = ', param_tau_reio

    ! Parameters for massive neutrinos                                                                                            

    write(UNIT_FILE2,'(a7, es16.10)') 'N_ur = ', param_N_ur

    write(UNIT_FILE2,'(a9, es16.10)') 'N_ncdm = ', param_N_ncdm

    write(UNIT_FILE2,'(a11, es16.10)') 'deg_ncdm = ', param_deg_ncdm

    write(UNIT_FILE2,'(a9, es16.10)') 'm_ncdm = ', param_m_ncdm
    ...
    write(UNIT_FILE2,'(a25)') 'dNdz_selection = analytic'
...
!    z_bin_bias(9),',',z_bin_bias(10)
    ...
    write(UNIT_FILE2,'(a12,i4)') 'l_max_lss = ', lmax

    write(UNIT_FILE2,'(a8,i1)') 'l_min = ', lmin

    write(UNIT_FILE2,'(a9,f2.0)') 's_bias = ', 0.

    write(UNIT_FILE2,'(a13)') 'format = camb'
...
subroutine write_ini_file_for_cmb(parameter_name, parameter_value)
    ...
    If (fiducial_flag) then
       ...
       open(UNIT_FILE2, file='./ini_files/Cl_fiducial_fake_planck.ini')

       write(UNIT_FILE2,*) 'root = ../data/Cl_fiducial_fake_planck_'
    ...
    If (parameter_name .ne. param_name_A_s) then 
       ...
       write(UNIT_FILE2,'(a6, es16.10)') 'A_s = ', parameter_value  
    ...
    If (parameter_name .ne. param_name_n_s) then
       ...
       write(UNIT_FILE2,'(a6, es16.10)') 'n_s = ', parameter_value
    ...
    Else
       ...
       write(UNIT_FILE2,'(a5, es16.10)') 'H0 = ', parameter_value
    ...
    Else
       ...
       write(UNIT_FILE2,'(a10, es16.10)') 'omega_b = ', parameter_value
    ...
    Else
       ...
       write(UNIT_FILE2,'(a12, es16.10)') 'omega_cdm = ', parameter_value
    ...
    Else
       ...
       write(UNIT_FILE2,'(a9, es16.10)') 'm_ncdm = ', parameter_value
    ...
    If (number_of_parameters .gt. 7) then
       ...
       If ( (DEA_MODEL .eq. 2) .or. (DEA_MODEL .eq. 3) ) then
          ...
          If (parameter_name .ne. param_name_g_pi) then
             ...
             write(UNIT_FILE2,'(a7, es16.10)') 'g_pi = ', 1.d1**g_pi
    ...
    write(UNIT_FILE2,'(a16)') 'output = tCl,pCl'

    write(UNIT_FILE2,'(a13)') 'headers = yes'

    write(UNIT_FILE2,'(a17)') 'bessel file = yes'

    write(UNIT_FILE2,'(a16,i4)') 'l_max_scalars = ', lmax_class_cmb
    ...
    write(UNIT_FILE2,'(a13)') 'lensing = yes'
...
subroutine write_ini_file_mcmc_for_cmb(parameter_vector,job)
    ...
    write(UNIT_FILE2,*) 'root = ../output/current_fake_planck_'//trim(job)//'_'
...
subroutine write_ini_file_for_fisher(parameter_name, parameter_value, lensing_flag, Cl_flag,bessel,q,kmaxtau0)
    ...
    If (fisher_analysis_at_bestfit) then
       ...
       If (lensing_flag) then
          ...
          If (Cl_flag) then
             ...
             If (bestfit_flag) then
                ...
                open(UNIT_FILE2, file='./ini_files/Bl_bestfit_lensing.ini')

                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'

                write(UNIT_FILE2,*) 'root = ../data/Bl_bestfit_lensing_'

             Else 

                open(UNIT_FILE2, file='./ini_files/Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_lensing.ini')

                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'

                write(UNIT_FILE2,*) 'root = ../data/Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_lensing_'

             End If

          Else

             open(UNIT_FILE2, file='./ini_files/Bl.ini')

             write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'

             write(UNIT_FILE2,*) 'root = ../data/Bl_'
       ...
       Else 
          ...
          If (Cl_flag) then
             ...
             If (bestfit_flag) then
                ...
                open(UNIT_FILE2, file='./ini_files/Bl_bestfit_no_lensing.ini')

                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'

                write(UNIT_FILE2,*) 'root = ../data/Bl_bestfit_no_lensing_'

             Else

                open(UNIT_FILE2, file='./ini_files/Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_no_lensing.ini')

                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'

                write(UNIT_FILE2,*) 'root = ../data/Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_no_lensing_'

             End If

          Else

             open(UNIT_FILE2, file='./ini_files/Bl_nl.ini')

             write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'

             write(UNIT_FILE2,*) 'root = ../data/Bl_nl_'
       ...
       If (parameter_name .ne. param_name_omega_b) then
          ...
          write(UNIT_FILE2,'(a10, es16.10)') 'omega_b = ', bestfit(1) 
       ...
       If (parameter_name .ne. param_name_omega_cdm) then
          ...
          write(UNIT_FILE2,'(a12, es16.10)') 'omega_cdm = ', bestfit(2)
       ...
       If (parameter_name .ne. param_name_n_s) then
          ...
          write(UNIT_FILE2,'(a6, es16.10)') 'n_s = ', bestfit(3)
       ...
       If (parameter_name .ne. param_name_A_s) then 
          ...
          write(UNIT_FILE2,'(a6, es16.10)') 'A_s = ', bestfit(4) 
       ...
       If (parameter_name .ne. param_name_H0) then
          ...
          write(UNIT_FILE2,'(a5, es16.10)') 'H0 = ', bestfit(5)
       ...
       If (parameter_name .ne. param_name_m_ncdm) then
          ...
          write(UNIT_FILE2,'(a9, es16.10)') 'm_ncdm = ', bestfit(6)
...
!!$
!!$          write(UNIT_FILE2,'(a11, es16.10)') 'MG_beta2 = ', MG_beta2 !bestfit(7)
!!$
!!$       Else
!!$
!!$          write(UNIT_FILE2,'(a11, es16.10)') 'MG_beta2 = ', MG_beta2 ! parameter_value
!!$ 
!!$       End If
       ...
       If (lensing_flag) then
          ...
          If (Cl_flag) then
             ...
             If (fiducial_flag) then
                ...
                open(UNIT_FILE2, file='./ini_files/Cl_fiducial_lensing.ini')

                write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'

                write(UNIT_FILE2,*) 'root = ../data/Cl_fiducial_lensing_'
          ...
          Else
             ...
             open(UNIT_FILE2, file='./ini_files/El.ini')

             write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'

             write(UNIT_FILE2,*) 'root = ../data/El_'
       ...
       Else 
          ...
          If (Cl_flag) then
             ...
             If (fiducial_flag) then
                ...
                open(UNIT_FILE2, file='./ini_files/Cl_fiducial_no_lensing.ini')

                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'

                write(UNIT_FILE2,*) 'root = ../data/Cl_fiducial_no_lensing_'

             Else

                open(UNIT_FILE2, file='./ini_files/Cl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_no_lensing.ini')

                write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'

                write(UNIT_FILE2,*) 'root = ../data/Cl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_no_lensing_'
...
!    write(UNIT_FILE2,'(a27,f2.0)') 'selection_magnitude_bias = ', 0.
    ...
    write(UNIT_FILE2,'(a40, f6.0)') 'l_switch_limber_for_nc_local_over_z =   ', real(l_switch_limber_for_nc_local_over_z)

    write(UNIT_FILE2,'(a40, f6.0)') 'l_switch_limber_for_nc_los_over_z =     ', real(l_switch_limber_for_nc_los_over_z)
...
subroutine write_ini_file_mcmc(parameter_vector, param_N_ur, param_N_ncdm,param_deg_ncdm,&
    ...
    logical :: len_flag
    character*16 :: string_omega_b, string_omega_cdm, string_n_s, string_A_s, string_H0, string_m_ncdm,fmt
    character*16 :: string_g_pi, string_nc_bias_b0
    ...
    If (len_flag) then 
        open(UNIT_FILE2, file='./ini_files/current_euclid_galaxy_cl_lensing_'//trim(job)//'.ini')
        write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
        write(UNIT_FILE2,*) 'root = ../output/current_euclid_galaxy_lensing_'//trim(job)//'_'
    else 
        open(UNIT_FILE2, file='./ini_files/current_euclid_galaxy_cl_'//trim(job)//'.ini')
        write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'
        write(UNIT_FILE2,*) 'root = ../output/current_euclid_galaxy_cl_'//trim(job)//'_'
    End if
    ...
    write(UNIT_FILE2,'(a13, es16.10)') 'nc_bias_b0 = ', param_nc_bias_b0
    ...
    write(UNIT_FILE2,'(a7, f5.3)') 'N_ur = ', real(param_N_ur)

    write(UNIT_FILE2,'(a9, f5.3)') 'N_ncdm = ', real(param_N_ncdm)

    write(UNIT_FILE2,'(a11, f5.3)') 'deg_ncdm = ', real(param_deg_ncdm)
...
subroutine write_ini_file(param_omega_b, param_omega_cdm, param_n_s, param_A_s, param_H0, &
    ...
    If (len_flag) then 
        open(UNIT_FILE2, file='./ini_files/current_euclid_galaxy_cl_lensing.ini')
        write(UNIT_FILE2,'(a59)') 'number count contributions = density, rsd, lensing, doppler'
        write(UNIT_FILE2,*) 'root = ../output/current_euclid_galaxy_lensing_'
    else 
        open(UNIT_FILE2, file='./ini_files/current_euclid_galaxy_cl_.ini')
        write(UNIT_FILE2,'(a50)') 'number count contributions = density, rsd, doppler'
        write(UNIT_FILE2,*) 'root = ../output/current_euclid_galaxy_'
    End if
...
subroutine fill_parameters_array(p)
  ...
  If (compute_data_fisher_analysis) then
     ...
     If (fisher_analysis_at_bestfit) then
        ...
        call write_ini_file_for_fisher('omega_b',bestfit(1),.false.,.false.,&
             selection_sampling_bessel_fid,q_linstep_fid,k_max_tau0_over_l_max_fid)    

        call write_ini_file_for_fisher('omega_b',bestfit(1),.true.,.false.,&
             selection_sampling_bessel_fid,q_linstep_fid,k_max_tau0_over_l_max_fid)    
     ...
     Else
        ...
        call write_ini_file_for_fisher('m_ncdm',m_ncdm,.true.,.false.,&
             selection_sampling_bessel_fid,q_linstep_fid,k_max_tau0_over_l_max_fid)

        call write_ini_file_for_fisher('m_ncdm',m_ncdm,.false.,.false.,&
             selection_sampling_bessel_fid,q_linstep_fid,k_max_tau0_over_l_max_fid)
...
subroutine run_class(parameter_name,parameter_value,lensing_flag,Cl_flag)
    ...
    character(len=*) :: parameter_name
    character*16 :: string_par_value
    logical :: exist,lensing_flag,Cl_flag,fid1,fid2,fid3,fid4,fid5,fid6,fiducial_flag,bestfit_flag
    character(len=*),parameter :: fmt = '(es16.10)'
       ...
       If (lensing_flag) then
          ...
          If (Cl_flag) then
             ...
             If (bestfit_flag) then
                ...
                inquire(file='./data/Bl_bestfit_lensing_cl.dat',exist=exist)

                If (.not.exist) then

                   call write_sh_file('Bl_bestfit_lensing')

                   call system('cd class_montanari-lensing ; sbatch Bl_bestfit_lensing.sh')

                End If

             Else 

                inquire(file='./data/Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_lensing_cl.dat',exist=exist)

                If (.not.exist) then

                   call write_sh_file('Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_lensing')

                   call system('cd class_montanari-lensing ; sbatch Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//&
                        '_lensing.sh')
          ...
          Else
             ...
             inquire(file='./data/Bl_cl.dat',exist=exist)

             If (.not.exist) then

                call write_sh_file('Bl')

                call system('cd class_montanari-lensing ; sbatch Bl.sh')
       ...
       Else 
          ...
          If (Cl_flag) then
             ...
             If (bestfit_flag) then
                ...
                inquire(file='./data/Bl_bestfit_no_lensing_cl.dat',exist=exist)

                If (.not.exist) then

                   call write_sh_file('Bl_bestfit_no_lensing')

                   call system('cd class_montanari-lensing ; sbatch Bl_bestfit_no_lensing.sh')

                End If

             Else

                inquire(file='./data/Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_no_lensing_cl.dat',exist=exist)

                If (.not.exist) then

                   call write_sh_file('Bl_'//trim(parameter_name)//'_'//trim(string_par_value)//'_no_lensing')
          ...
          Else
             ...
             inquire(file='./data/Bl_nl_cl.dat',exist=exist)

             If (.not.exist) then

                call write_sh_file('Bl_nl')

                call system('cd class_montanari-lensing ; sbatch Bl_nl.sh')
...
subroutine compute_b_lambda_alpha()
    ...
    open(UNIT_FILE1,file='./output/b_lambda_alpha.dat')

    write(UNIT_FILE1,*) '# Order of parameters in columns is as follows: '
    write(UNIT_FILE1,*) '# omega_b, omega_cdm, n_s, ln(10^10*A_s), H_0, m_ncdm, MG_beta2 '
    write(UNIT_FILE1,'(7es25.10)') b_lambda(1), b_lambda(2), b_lambda(3),&
...
subroutine compute_inverse_fisher_matrix()
    ...
    open(UNIT_FILE1,file='./output/inverse_fisher_matrix.dat')
    write(UNIT_FILE1,*) '# Order of parameters in rows and columns is as follows: '
    write(UNIT_FILE1,*) '# omega_b, omega_cdm, n_s, ln(10^10*A_s), H_0, m_ncdm, MG_beta2 '
    write(UNIT_FILE1,'(7es25.10)') inv_F_ab(1,1), inv_F_ab(1,2), inv_F_ab(1,3), inv_F_ab(1,4),&
         inv_F_ab(1,5), inv_F_ab(1,6), inv_F_ab(1,7)
    write(UNIT_FILE1,'(7es25.10)') inv_F_ab(2,1), inv_F_ab(2,2), inv_F_ab(2,3), inv_F_ab(2,4),&
         inv_F_ab(2,5), inv_F_ab(2,6), inv_F_ab(2,7)
    write(UNIT_FILE1,'(7es25.10)') inv_F_ab(3,1), inv_F_ab(3,2), inv_F_ab(3,3), inv_F_ab(3,4),&
         inv_F_ab(3,5), inv_F_ab(3,6), inv_F_ab(3,7)
    write(UNIT_FILE1,'(7es25.10)') inv_F_ab(4,1), inv_F_ab(4,2), inv_F_ab(4,3), inv_F_ab(4,4),&
         inv_F_ab(4,5), inv_F_ab(4,6), inv_F_ab(4,7)
    write(UNIT_FILE1,'(7es25.10)') inv_F_ab(5,1), inv_F_ab(5,2), inv_F_ab(5,3), inv_F_ab(5,4),&
         inv_F_ab(5,5), inv_F_ab(5,6), inv_F_ab(5,7)
    write(UNIT_FILE1,'(7es25.10)') inv_F_ab(6,1), inv_F_ab(6,2), inv_F_ab(6,3), inv_F_ab(6,4),&
         inv_F_ab(6,5), inv_F_ab(6,6), inv_F_ab(6,7)
    write(UNIT_FILE1,'(7es25.10)') inv_F_ab(7,1), inv_F_ab(7,2), inv_F_ab(7,3), inv_F_ab(7,4),&
...
subroutine compute_fisher_matrix(lensing_flag,autocorr_flag)
    ...
    If (lensing_flag) then 
        ...
        If (autocorr_flag) then
            open(UNIT_FILE1,file='./output/fisher_matrix_lensing_only_autocorrelations.dat')
        Else
            open(UNIT_FILE1,file='./output/fisher_matrix_lensing.dat')
        End If
        write(UNIT_FILE1,*) '# Order of parameters in rows and columns is as follows: '
        write(UNIT_FILE1,*) '# omega_b, omega_cdm, n_s, ln(10^10*A_s), H_0, m_ncdm, MG_beta2 '
        write(UNIT_FILE1,'(7es25.10)') F_ab(1,1), F_ab(1,2), F_ab(1,3), F_ab(1,4), F_ab(1,5), F_ab(1,6), F_ab(1,7)
        write(UNIT_FILE1,'(7es25.10)') F_ab(2,1), F_ab(2,2), F_ab(2,3), F_ab(2,4), F_ab(2,5), F_ab(2,6), F_ab(2,7)
        write(UNIT_FILE1,'(7es25.10)') F_ab(3,1), F_ab(3,2), F_ab(3,3), F_ab(3,4), F_ab(3,5), F_ab(3,6), F_ab(3,7)
        write(UNIT_FILE1,'(7es25.10)') F_ab(4,1), F_ab(4,2), F_ab(4,3), F_ab(4,4), F_ab(4,5), F_ab(4,6), F_ab(4,7)
        write(UNIT_FILE1,'(7es25.10)') F_ab(5,1), F_ab(5,2), F_ab(5,3), F_ab(5,4), F_ab(5,5), F_ab(5,6), F_ab(5,7)
        write(UNIT_FILE1,'(7es25.10)') F_ab(6,1), F_ab(6,2), F_ab(6,3), F_ab(6,4), F_ab(6,5), F_ab(6,6), F_ab(6,7)
        write(UNIT_FILE1,'(7es25.10)') F_ab(7,1), F_ab(7,2), F_ab(7,3), F_ab(7,4), F_ab(7,5), F_ab(7,6), F_ab(7,7)
        close(UNIT_FILE1)
    else 
...
subroutine compute_B_beta()
    ...
    open(UNIT_FILE1,file='./output/B_beta.dat')

    write(UNIT_FILE1,*) '# Order of parameters in columns is as follows: '
    write(UNIT_FILE1,*) '# omega_b, omega_cdm, n_s, ln(10^10*A_s), H_0, m_ncdm, MG_beta2 '
    write(UNIT_FILE1,'(7es25.10)') B_beta(1), B_beta(2), B_beta(3), B_beta(4), B_beta(5), B_beta(6), B_beta(7)
